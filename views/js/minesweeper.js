...
function init(w, h, t, m) {
	...
	var game_row = `<td class='sq' ID='sq-${i}' onclick='clickSq(event,${i})'>&nbsp;</TD>`
	
	for (var i = 0; i < h; i++){
		$('#game_row').append(`<tr id='row-${i}'></tr>`)
		for (var j = 0; j < w; j++){
			var game_col = (`<td class='sq' id='sq-${cnt}' onclick='clickSq(event,${cnt})'>&nbsp;</TD>`)
			$(`#row-${i}`).append(game_col)
			console.log(game_col)
			cnt ++
		}
	}
